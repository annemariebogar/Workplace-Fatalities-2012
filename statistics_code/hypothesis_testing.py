...
t_table = {1:[1.000, 1.376, 1.963, 3.078, 6.314, 12.71, 31.82, 63.66, 318.31, 636.62],
...
50:[0.679, 0.849, 1.047, 1.295, 1.676, 2.009, 2.403, 2.678, 3.261, 3.496],
...
z_table = [0.674, 0.842, 1.036, 1.282, 1.645, 1.960, 2.326, 2.576, 3.090, 3.291]
sig_level = [0.25, 0.2, 0.15, 0.1, 0.05, 0.025, 0.01, 0.005, 0.001, 0.0005]
confidence_level = [0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.98, 0.99, 0.998, 0.999]

#tail = 1, 2, 0(confidence interval)
def t_test(num, level, tail=0):
    df = num-1
    if tail == 0 and level in confidence_level:
            return t_table[df][confidence_level.index(level)]
    elif (tail == 1 or tail == 2) and level/tail in sig_level:
        return t_table[df][sig_level.index(level/tail)]
    else:
        return 0

def z_test(level):
    if level < 0.5 and (level/2) in sig_level:
        return z_table[sig_level.index(level/2)]
    elif level in confidence_level:
        return z_table[confidence_level.index(level)]
    else:
        return 0

def reject(critical, statistic):
    print("critical value: " + str(critical))
    print("test stat: " + str(statistic))
    if abs(critical) > abs(statistic):
        return False
    return True

#st = (sample mean - accepted mean) / (sample sd / square root of n)
def test_stat(sample, population):
    sample_mean = statistics.mean(sample)
    print("sample mean: " + str(sample_mean))
    accepted_mean = statistics.mean(population)
    print("accepted mean: " + str(accepted_mean))
    ...
    return (sample_mean - accepted_mean) / standard_error

#ci = [mean-(critical * (sd / square root n))
def confidence_interval(data, level):
    mean = statistics.mean(data)
    standard_deviation = statistics.stdev(data)
    standard_error = standard_deviation / math.sqrt(n)
    if len(data) < 30:
        critical = t_test(len(data), level, 0)
    else:
        critical = z_test(level)
